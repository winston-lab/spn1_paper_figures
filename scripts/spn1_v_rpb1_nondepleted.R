main = function(spn1_path="depleted-v-non-depleted_Spn1-chipseq-spikenorm-verified-coding-genes-diffbind-results-all.tsv",
    ...
    pearson = cor(df["control_enrichment_rpb1"],
                  df["control_enrichment_spn1"],
                  use="complete.obs")

    spn1_v_rpb1_nondepleted = ggplot(data=df,
        ...
        stat_binhex(geom="point",
                    ...
                    alpha=0.8) +
        annotate(geom="text",
                 x=min(df[["control_enrichment_rpb1"]], na.rm=TRUE),
                 y=max(df[["control_enrichment_spn1"]], na.rm=TRUE),
        ...
        scale_y_continuous(name="non-depleted\nSpn1 enrichment") +
